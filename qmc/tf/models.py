...
class QMClassifier(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        #tf.print('***call_psi_x***', psi_x)
        rho_y = self.qm(psi_x)
        #tf.print('***call_rho_y***', rho_y)
        probs = self.dm2dist(rho_y)
        #tf.print('***call_probs***', probs)
        return probs
    ...
    def call_train(self, x, y):
        ...
        if not self.qm.built:
            self.call(x)
        psi_x = self.fm_x(x)
        #tf.print('***call_train_psi_x***', psi_x)
        rho_y = self.qm(psi_x)
        #tf.print('***call_train_rho_y***', rho_y)
        psi_y = self.fm_y(y)
        #tf.print('***call_train_psi_y***', psi_y)
        psi = self.cp1([psi_x, psi_y])
        #tf.print('***call_train_psi***', psi)
        rho = self.cp2([psi, tf.math.conj(psi)])
        #tf.print('***call_train_rho_shape***', rho.shape)
        #tf.print('***call_train_rho***', rho)
        
        num_samples = tf.cast(tf.shape(x)[0], rho.dtype)
        #tf.print('***call_train_num_samples***', num_samples)
