...
class QMeasureClassif(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        #tf.print("layer_call_inputs_shape: ", inputs.shape)
        #tf.print("layer_call_inputs: ", inputs)
        oper = tf.einsum(
            '...i,...j->...ij',
            inputs, tf.math.conj(inputs),
            optimize='optimal') # shape (b, nx, nx)
        #tf.print("layer_call_oper_shape: ", oper.shape)
        #tf.print("layer_call_oper: ", oper)
        #tf.print("layer_call_rho_shape: ", self.rho.shape)
        #tf.print("layer_call_rho: ", self.rho)

        rho_res = tf.einsum(
            '...ik, klmn, ...mo -> ...ilon',
            oper, self.rho, oper,
            optimize='optimal')  # shape (b, nx, ny, ny, nx)
        #tf.print("rho:", self.rho)
